...
/*C_755E*/CMD_Locate()
{
	u4_puts(/*D_22EA*/&AVATAR[0x1159D] /* "Locate position with " */);
	if(Party._sextants == 0) {
		w_What();
		return 0;
	}
	u4_puts(/*D_2300*/&AVATAR[0x115B3] /* "sextant\n" */);
	add_char_to_text_buffer('\n');
	u4_puts(&AVATAR[0x115BC] /* "Latitude: " */);
...
unsigned char * D_2334 = &AVATAR[0x115E7]; //{
#if 0
	/*weapons class masks*/
...
int bp04;
{
	u4_puts(/*D_2322*/&AVATAR[0xF42B] /* "\nA " */);
	u4_puts(D_1E98[77 + (int)Party.chara[bp08]._class]);
	u4_puts(/*D_2326*/&AVATAR[0x115D9] /* " may NOT use " */);
	if (bp04)
	{
		u4_putc('a');
...
/*C_7631*/CMD_Ready()
{
	...
	if(CurMode >= MOD_COMBAT) {
		u4_puts(/*D_234C*/&AVATAR[0x115FF] /* "Ready Weapon!\n" */);
		loc_A = activeChara;
	} else {
		u4_puts(/*D_235B*/&AVATAR[0x1160E] /* "Ready a weapon\n" */);
		set_input_mode(INPUT_MODE_GENERAL_ASK_CHARACTER_NUMBER);
		if((loc_A = AskChara(/*D_236B*/&AVATAR[0x10C9C] /* "for:\x12\x12\b" */)) < 0)
			return 0;
	}
	Gra_13();
	C_4832();
	...
	loc_B = AskLetter(/*D_2373*/&AVATAR[0x11626] /* "Weapon:\x12\x12\b" */, 'A', 'P');
	Gra_13();
	C_4649();
	dspl_Stats();
	if(CurMode >= MOD_COMBAT)
...
/*C_7732*/CMD_Wear()
{
	...
	u4_puts(/*D_237E*/&AVATAR[0x11631] /* "Wear Armour\n" */);
	set_input_mode(INPUT_MODE_GENERAL_ASK_CHARACTER_NUMBER);
	if((loc_A = AskChara(/*D_238B*/&AVATAR[0x10C9C] /* "for:\x12\x12\b" */)) < 0)
		return 0;
	Gra_13();
	C_48F8();
	txt_Y = 23; txt_X = 24;
	set_input_mode(INPUT_MODE_GENERAL_ASK_LETTER_ARMOR);
	loc_B = AskLetter(/*D_2393*/&AVATAR[0x11646] /* "Armour:\x12\x12\b" */, 'A', 'H');
	Gra_13();
	C_4649();
	dspl_Stats();
	if(CurMode >= MOD_COMBAT)
