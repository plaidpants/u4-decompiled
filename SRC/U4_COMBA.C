...
/*C_5A9E*/
			if(Fighters._chtile[activeChara] && isCharaConscious(activeChara)) {
				...
				u4_puts(/*D_1FE6*/&AVATAR[0x11299] /* " with " */);
				...
				switch(si) {
					...
					default:
						u4_puts(/*D_1FED*/&AVATAR[0x112A0] /* "Bad command\n" */);
						sound(2,0);
						Gra_11(activeChara);
						activeChara --;
					continue;
...
unsigned char _damage;
{
	...
	if(Fighters._tile[_npcId] != TIL_5E && (Fighters._HP[_npcId] -= _damage) < 0) {
		u4_puts(/*D_1FFA*/&AVATAR[0x112AD] /* "Killed!\n" */);
		if(_charaId != -1) {
			register int di;

			di = D_23D2[C_7C25(Fighters._tile[_npcId])] / 16 + 1;
			XP_inc(_charaId, di);
			u4_puts(/*D_2003*/&AVATAR[0x112B6] /* "Exp. " */);
			u4_putl(di, 1, ' ');
			Gra_CR();
		}
		...
		if(loc_D < 24) {
			u4_puts(/*D_2009*/&AVATAR[0x112BC] /* "Fleeing!\n" */);
		} else if(loc_D < loc_B) {
			u4_puts(/*D_2013*/&AVATAR[0x112C6] /* "Critical!\n" */);
		} else {
			Gra_CR();
			if(loc_D < loc_A) {
				u4_puts(/*D_201E*/&AVATAR[0x112D1] /* "Heavily " */);
			} else if(loc_D < loc_C) {
				u4_puts(/*D_2027*/&AVATAR[0x112DA] /* "Lightly " */);
			} else {
				u4_puts(/*D_2030*/&AVATAR[0x112E3] /* "Barely " */);
			}
			u4_puts(/*D_2038*/&AVATAR[0x112EB] /* "Wounded!\n" */);
		}
	}
}
...
int /*bp04*/_range;
{
	if(_range == 0)
		sound(4,0);
	hit_tile = 0;
	u4_puts(/*D_2042*/&AVATAR[0x112F5] /* "Missed!\n" */);
...
int _dir_y;
{
	...
	if(loc_A->_weapon == 9) {
		...
		loc_D = AskLetter(/*D_204B*/&AVATAR[0x112FE] /* "Range:\x12\x12\b" */, '0', '9');
		loc_D -= '0';
		if(loc_D < 0)
			return 0;
	}
	...
	if(loc_A->_weapon == 2 || loc_A->_weapon == 9) {
		if(Party._weapons[loc_A->_weapon] == 0) {
			loc_A->_weapon = 0;
			u4_puts(/*D_2055*/&AVATAR[0x11308] /* "Last one!\n" */);
		} else {
...
C_61D1()
{
	...
	AskDir(/*D_2060*/&AVATAR[0x10AE0] /* "Dir: " */, &loc_A, &loc_B);
	if(!(loc_A | loc_B))
		return 0;
...
/*C_628F*/CMD_Attack()
{
	...
	if(CurMode <= MOD_BUILDING && Party.f_1dc != 0) {
		u4_puts(/*D_2066*/&AVATAR[0x11319] /* "Attack\n" */);
		w_DriftOnly();
		return 0;
	}

	set_input_mode(INPUT_MODE_GENERAL_DIRECTION);
	AskDir(/*D_206E*/&AVATAR[0x11321] /* "Attack: " */, &loc_A, &loc_B);
	if(!(loc_A|loc_B))
		return 0;
	if(
		...
		u4_puts(/*D_2077*/&AVATAR[0x1132A] /* "Nothing to Attack!\n" */);
		return 0;
	}
