...
unsigned char * D_277E = &AVATAR[0x11A29 + 0x2017]; // {
#if 0
...
/*C_8C08*/CMD_Mix()
{
	...
	do {
		...
		txt_Y = 23;
		txt_X = 24;
		u4_puts(/*D_270C*/&AVATAR[0x119B7 + 0x2017] /* "Mix Reagents\n" */);
		set_input_mode(INPUT_MODE_GENERAL_ASK_SPELL);
		if((loc_C = AskLetter(/*D_271A*/&AVATAR[0x119C5 + 0x2017] /* "For Spell:\x12\x12\b" */, 'A', 'Z')) < 0)
			break;
		loc_C -= 'A';
		...
		while(1) {
			...
			if((loc_B = AskLetter(/*D_2728*/&AVATAR[0x119D3 + 0x2017] /* "Reagent:\x12\x12\b" */, 'A', 'H')) == -2) {
				for(si = 7; si >= 0; si --)
			...
			if(loc_B == -1) {
				if(loc_E == 0) {
					u4_puts(/*D_2734*/&AVATAR[0x119DF + 0x2017] /* "\nNothing mixed!\n" */);
				} else {
					u4_puts(/*D_2745*/&AVATAR[0x119F0 + 0x2017] /* "\nYou mix the Reagents, and...\n" */);
					loc_B = 0;
					if(D_277E[loc_C] == loc_E) {
						u4_puts(/*D_2764*/&AVATAR[0x11A0F + 0x2017] /* "Success!\n\n" */);
						Party._mixtures[loc_C] ++;
						if(Party._mixtures[loc_C] > 99)
							Party._mixtures[loc_C] = 99;
					} else {
						u4_puts(/*D_276F*/&AVATAR[0x11A1A + 0x2017] /* "It Fizzles!\n\n" */);
					}
				}
				break;
			}
