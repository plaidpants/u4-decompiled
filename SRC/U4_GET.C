...
C_70F1()
{
	...
	u4_puts(/*D_2240*/&AVATAR[0x114F3] /* "The Chest Holds:\n" */);
	si = U4_RND3(80) + U4_RND1(7) + 10;
	u4_putl(si, 1, ' ');
	u4_puts(/*D_2252*/&AVATAR[0x11505] /* " Gold\n" */);
...
int bp04;
{
	...
	if(((si = U4_RND1(3))&1) == 0) {
		...
		si &= u_rand_a();
		if(si == 0)
			u4_puts(/*D_2259*/&AVATAR[0x1150C] /* "Acid" */);
		else if(si == 1)
			u4_puts(/*D_225E*/&AVATAR[0x10FEE] /* "Sleep" */);
		else if(si == 2)
			u4_puts(/*D_2264*/&AVATAR[0x11517] /* "Poison" */);
		else
			u4_puts(/*D_226B*/&AVATAR[0x1151E] /* "Bomb" */);
		u4_puts(/*D_2270*/&AVATAR[0x11523] /* " Trap!\n" */);
		if(bp04 == -1 || U4_RND3(100) <= Party.chara[bp04]._dex + 25) {
			u4_puts(/*D_2278*/&AVATAR[0x1152B] /* "Evaded!\n" */);
			sound(8,0);
		} else if(si == 0) {/*Acid*/
			C_70CE(bp04);
		} else if(si == 1) {/*Sleep*/
...
/*C_72EC*/CMD_Get()
{
	...
	if(Party.f_1dc != 0 && Party._tile == TIL_18) {
		u4_puts(/*D_2281*/&AVATAR[0x11534] /* "Get chest\n" */);
		w_DriftOnly();
		return 0;
	}
	u4_puts(/*D_228C*/&AVATAR[0x1153F] /* "Get Chest!\n" */);
	set_input_mode(INPUT_MODE_GENERAL_ASK_CHARACTER_NUMBER);
	if((si = AskChara(/*D_2298*/&AVATAR[0x1154B] /* "Who opens?\x12\x12\b" */)) < 0)
		return 0;
...
C_73B8()
{
	u4_puts(/*D_22A6*/&AVATAR[0x1153F] /* "Get Chest!\n" */);
	C_7337();
}
...
char * D_22C8 = &AVATAR[0x1157B] /* "Broadsides Only!\n" */;

/*C_73C9*/CMD_Fire()
{
	int loc_A, loc_B, loc_C, loc_D, loc_E, loc_F;

	u4_puts(/*D_22B2*/&AVATAR[0x11565] /* "Fire " */);
	if(Party._tile < TIL_10 || Party._tile > TIL_13) {
		w_What();
		return 0;
	}
	u4_puts(/*D_22B8*/&AVATAR[0x1156B] /* "Cannon!\n" */);
	set_input_mode(INPUT_MODE_GENERAL_DIRECTION);
	AskDir(/*D_22C1*/&AVATAR[0x10AE0] /* "Dir: " */, &loc_C, &loc_D);
	if(!(loc_C|loc_D))
		return 0;
	if(loc_C != 0) {
...
/*C_7525*/CMD_Ignite()
{
	u4_puts(/*D_22DA*/&AVATAR[0x1158D] /* "Ignite Torch!\n" */);
	if(CurMode != MOD_DUNGEON) {
		w_NotHere();
		return 0;
	}
