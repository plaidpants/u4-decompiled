...
/*C_0CF7*/dspl_Stats()
{
	...
	if(Party._tile < TIL_14) {
		u4_puts(&AVATAR[0xF7AC] /* " SHP:" */);
		u4_putl(Party._ship, 2, '0');
	} else {
		u4_puts(&AVATAR[0xF7B2] /* " G:" */);
...
C_0EB1()
{
	...
	set_input_mode(INPUT_MODE_DELAY_NO_CONTINUE);
	u_delay(5, 0);
	u4_puts(&AVATAR[0xF7D7] /* "Where am I?...\n" */);
	set_input_mode(INPUT_MODE_DELAY_NO_CONTINUE);
	u_delay(5, 0);
	u4_puts(&AVATAR[0xF7E7] /* "Am I dead?...\n" */);
	set_input_mode(INPUT_MODE_DELAY_NO_CONTINUE);
	u_delay(5, 0);
	u4_puts(&AVATAR[0xF7F6] /* "Afterlife?...\n" */);
	set_input_mode(INPUT_MODE_DELAY_NO_CONTINUE);
	u_delay(5, 0);
	u4_puts(&AVATAR[0xF805] /* "You hear:\n" */);
	txt_X = (12 - (unsigned)strlen(Party.chara[0]._name)) / 2 + 25;
	u4_puts(Party.chara[0]._name);
	Gra_CR();
	set_input_mode(INPUT_MODE_DELAY_NO_CONTINUE);
	u_delay(5, 0);
	u4_puts(&AVATAR[0xF810] /* "I feel motion...\n" */);
	set_input_mode(INPUT_MODE_DELAY_NO_CONTINUE);
	u_delay(5, 0);
	Party._tile = TIL_1F;
	...
	if(Party._loc == 0) {
		if(Save(&AVATAR[0xF822] /* "OUTMONST.SAV" */, sizeof(struct tNPC), &(D_8742._npc)) == -1)
			exit(3);
	}
	if(Load(&AVATAR[0xF82F] /* "LCB_2.ULT" */, sizeof(struct t_500), &D_8742) == -1)
		exit(3);
	//File_TLK = dopen(&AVATAR[0xF839] /* "LCB.TLK" */, 0);
...
/*C_1188*/w_What()
{
	u4_puts(&AVATAR[0xF8B4] /* "What?\n" */);
	u_kbflush();
}

/*C_1199*/w_Cant_t()
{
	u4_puts(&AVATAR[0xF8BB] /* "Can't!\n" */);
	u_kbflush();
}

/*C_11AA*/w_NotHere()
{
	u4_puts(&AVATAR[0xF8C3] /* "Not Here!\n" */);
	u_kbflush();
}

/*C_11BB*/w_Disabled()
{
	u4_puts(&AVATAR[0xF8CE] /* "Disabled!\n" */);
	u_kbflush();
}

/*C_11CC*/w_OnlyOnFoot()
{
	u4_puts(&AVATAR[0xF8D9] /* "Only on foot!\n" */);
	u_kbflush();
}

/*C_11DD*/w_SlowProgress()
{
	u4_puts(&AVATAR[0xF8E8] /* "Slow progress!\n" */);
}

/*C_11EB*/w_NoneLeft()
{
	u4_puts(&AVATAR[0xF8F8] /* "None left!\n" */);
}
	...
	*si = *di = 0;
	do {
		...
		switch(bp_04) {
			...
			case KBD_UP:
				*di = -1;
				u4_puts(&AVATAR[0xF907] /* "North" */);
			break;
			case KBD_DOWN:
				*di = 1;
				u4_puts(&AVATAR[0xF90D] /* "South" */);
			break;
			case KBD_RIGHT:
				*si = 1;
				u4_puts(&AVATAR[0xF913] /* "East" */);
			break;
			case KBD_LEFT:
				*si = -1;
				u4_puts(&AVATAR[0xF918] /* "West" */);
			break;
			default:
				bp_04 &= 0xff;
				if(bp_04 >= ' ' && bp_04 < 0x7f)
	...
	if(CurMode >= MOD_COMBAT || Party._tile > TIL_13) {
		...
		if((Party._ship -= 10) < 0) {
			...
			for(loc_B = Party.f_1d8; --loc_B >= 0; )
				Gra_11(loc_B);
			dspl_Stats();
			u4_puts(&AVATAR[0xF91D] /* "Thy Ship Sinks!\n" */);
			C_0EB1();
		}
	}
	dspl_Stats();
